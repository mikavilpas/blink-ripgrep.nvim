...
function RipgrepCommand.get_command(prefix, options)
  local cmd = {
    ...
    "--json",
    "--word-regexp",
    "--max-filesize=" .. options.future_features.backend.ripgrep.max_filesize,
    options.future_features.backend.ripgrep.search_casing,
  }

  for _, option in
    ipairs(options.future_features.backend.ripgrep.additional_rg_options)
  do
  ...
  if options.future_features.backend.ripgrep.project_root_fallback then
  ...
  for _, additional_root in
    ipairs(options.future_features.backend.ripgrep.additional_paths or {})
  do
